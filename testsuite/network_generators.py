...
def generate_rewired_network(ggi_network):
    ...
    """
    degree_sequence = [ggi_network.degree()[node] for node in ggi_network.nodes()]
    rewired_network = nx.random_degree_sequence_graph(degree_sequence, tries=1000)
...
def generate_shuffled_network(ggi_network):
...
def generate_scale_free_network(ggi_network):
...
def generate_uniform_network(ggi_network):
    ...
    num_nodes = ggi_network.number_of_nodes()
    num_edges = ggi_network.number_of_edges()
    uniform_network = nx.gnm_random_graph(num_nodes, num_edges)
    gene_ids = nx.get_node_attributes(ggi_network, utils.gene_id_attribute_name())
    nx.set_node_attributes(uniform_network, gene_ids, utils.gene_id_attribute_name())
    return uniform_network


def generate_network(ggi_network, network_generator_selector):
