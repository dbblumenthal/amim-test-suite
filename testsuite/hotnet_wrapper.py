...
class HotNetWrapper(AlgorithmWrapper):
    ...
    @staticmethod
    def permute_scores(path_scores, path_permuted_scores):
        hotnet = 'cd ../algorithms/hierarchical_hotnet/src; python permute_scores.py'
        command = f'{hotnet} -i ../{path_scores} -o ../{path_permuted_scores}'
    ...
    def run_algorithm(self, ggi_network, expression_data, phenotypes, seed_genes, p_values, indicator_matrix, prefix):
        ...
        args = list(itt.product([path_scores], paths_permuted_scores))
        pool.starmap(self.permute_network, args)
        ...
        args = list(itt.product([path_sim_matrix], [path_map],  [path_scores] + paths_permuted_scores,
                                [path_h_ggi] + paths_permuted_h_ggis, [path_h_map] + paths_permuted_h_maps))
        ...
        command = f'{hotnet} -oelf {path_h_ggi} -oigf {path_h_map} -pelf {" ".join(paths_permuted_h_ggis)} -pigf {" ".join(paths_permuted_h_maps)} -cf ../{path_output}'
        subprocess.call(command, shell=True, stdout=subprocess.PIPE)
