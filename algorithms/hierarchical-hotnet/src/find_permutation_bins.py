...
def run(args):
    ...
    # G = max(nx.connected_component_subgraphs(G), key=len) # testing-onfah
    # common_genes = set(G.nodes()) # testing-onfah
    common_genes = max(nx.connected_components(G), key=len)
    G = G.subgraph(common_genes)
